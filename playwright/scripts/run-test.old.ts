...
function render() {
    ...
    // Calculate how many lines can be shown
    const maxLines = term.height - 5;
    // We'll determine sticky folders after computing the visible slice
    // Center the cursor in the viewport
    let start = 0;
    let end = 0;
    {
        // We'll first guess visibleLines = maxLines, then adjust after stickyFolders are known
        start = Math.max(0, cursor - Math.floor(maxLines / 2));
        end = Math.min(visibleNodes.length, start + maxLines);
    }
    let visibleSlice = visibleNodes.slice(start, end);

    // Now, sticky folder calculation:
    // Only folders before 'start' that are not visible in the slice
    const stickyFolders: { node: TreeNode; depth: number }[] = [];
    const sliceNodes = visibleSlice.map((item) => item.node);
    for (let i = 0; i < start; i++) {
        const { node, depth } = visibleNodes[i];
        if (node.isFolder && node.isOpen) {
            // Only sticky if not in visible slice
            const inSlice = sliceNodes.includes(node);
            if (!inSlice) stickyFolders.push({ node, depth });
        }
    }

    // Now, recalculate visibleLines to account for stickyFolders
    const stickyCount = stickyFolders.length;
    const visibleLines = maxLines - stickyCount;
    start = Math.max(0, cursor - Math.floor(visibleLines / 2));
    end = Math.min(visibleNodes.length, start + visibleLines);
    visibleSlice = visibleNodes.slice(start, end);

    // Recalculate stickyFolders with new start/end and visibleSlice
    const visibleSliceNodes = visibleSlice.map((item) => item.node);
    const stickyFoldersFinal: { node: TreeNode; depth: number }[] = [];
    for (let i = 0; i < start; i++) {
        const { node, depth } = visibleNodes[i];
        if (node.isFolder && node.isOpen) {
            const inSlice = visibleSliceNodes.includes(node);
            if (!inSlice) stickyFoldersFinal.push({ node, depth });
        }
    }

    // Render sticky folders in order from shallowest to deepest
    stickyFoldersFinal.forEach(({ node, depth }) => {
        term(" " + "  ".repeat(depth));
        if (node.isFolder) {
            if (node.isOpen) {
                term.cyan("▼ ");
    ...
    // Render visible slice below sticky folders
    visibleSlice.forEach(({ node, depth }, idx) => {
...
async function main() {
    ...
    term.on("key", (name: string, matches: string[], data: any) => {
        if (name === "UP") {
            ...
            const target = Math.max(0, cursor - 5);
            cursor = cursor == 0 ? visibleNodes.length - 1 : target;
            render();
            return;
        } else if (name === "SHIFT_DOWN") {
            const target = Math.min(visibleNodes.length - 1, cursor + 5);
            cursor = cursor == visibleNodes.length - 1 ? 0 : target;
            render();
            return;
        } else if (name === "LEFT") {
            ...
            if (node.isFolder && node.isOpen) {
                ...
                for (let i = cursor - 1; i >= 0; i--) {
                    ...
                    if (
                        candidate.isFolder &&
                        candidate.isOpen &&
                        visibleNodes[i].depth < visibleNodes[cursor].depth
                    ) {
                        parentIndex = i;
                        break;
                    }
                ...
                if (parentIndex !== -1) {
                    visibleNodes[parentIndex].node.isOpen = false;
                    visibleNodes = flattenTree(
                        filterTreeByQuery(tree, searchQuery)
                    );
                    if (cursor >= visibleNodes.length) {
